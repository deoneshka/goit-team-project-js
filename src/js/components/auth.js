...
function openAuthnModal() {
  ...
  refs.authModal.firstElementChild.style.left =
...
function closeAuthModal() {
  ...
  refs.authModal.firstElementChild.style.left = '0px';
}
...
function authFormHeandler(event) {
  ...
  const authSignUpBtn = document.querySelector(
    '.authorization__registration-btn',
  );
  const authLoginBtn = document.querySelector('.authorization__login-btn');
  authSignUpBtn.addEventListener(
    'click',
    createNewUser(email, password, event),
  );
